  ...
  companion object {
    ...
    val `package` = Package(
      ...
      availableVersions = {
        ...
        client.getTags(REPO)
          .getOrElse { DEFAULT_VERSIONS.map { "cluster-autoscaler-" + it } }
          .mapNotNull { tag ->
            if (tag.startsWith("cluster-autoscaler-")) tag.removePrefix("cluster-autoscaler-") else null
          }
          .filter { !it.contains("-") && !it.contains("rc") }
          .distinct()
      },
      repoVersion = { v, toRepo -> if (toRepo) "cluster-autoscaler-$v" else v }
    )
  }
}
