...
class Dockerfile {
  ...
  // Fluent FROM methods
  fun from(image: String, tag: String? = null, alias: String? = null, platform: String? = null): Dockerfile {
  ...
  fun run(
    ...
    val instruction = buildString {
      ...
      mounts.forEach { mount ->
        append(" --mount=${mount.toMountString()}")
      }
  ...
  // Fluent COPY method
  fun copy(src: String, dest: String, from: String? = null, chown: String? = null, chmod: String? = null): Dockerfile {
    val instruction = buildString {
      append("COPY")
  ...
  // Fluent ADD method
  fun add(src: String, dest: String, chown: String? = null, chmod: String? = null, checksum: String? = null): Dockerfile {
    val instruction = buildString {
      append("ADD")
      chown?.let { append(" --chown=$it") }
      chmod?.let { append(" --chmod=$it") }
  ...
  fun env(block: EnvConfiguration.() -> Unit): Dockerfile {
    ...
    config.variables.forEach { (key, value) ->
      instructions.add("ENV $key=$value")
    }
    return this
  }
  ...
  fun healthcheck(block: HealthcheckConfiguration.() -> Unit): Dockerfile {
    ...
    return healthcheck(config.interval, config.timeout, config.startPeriod, config.retries, config.cmd)
  }
  ...
  data class RunConfiguration(
    ...
    fun bindMount(source: String, target: String, readonly: Boolean = false, from: String? = null) {
      mounts.add(BindMount(source, target, readonly, from))
    }

    fun cacheMount(target: String, id: String? = null, readonly: Boolean = false, sharing: CacheSharing = CacheSharing.SHARED) {
    ...
    fun secretMount(id: String, target: String? = null, required: Boolean = false, mode: String? = null) {
      mounts.add(SecretMount(id, target, required, mode))
    }

    fun sshMount(id: String = "default", target: String? = null, required: Boolean = false, mode: String? = null) {
