...
class OpenCostTest {
  ...
  fun testOpenCost() {
    runTest(timeout = 10.seconds) {
      // val latest = OpenCost.`package`.latestVersion().getOrElse { err -> throw Exception("Unable to determine latest version: $err") }

      // Environment.all.forEach { env ->
      //   PackageTestCase(
      //     "open cost",
      //     env,
      //     rendered = OpenCost(latest).render().getOrElse { err -> throw Exception("Unable to render: $err") },
      //   ).isExpected()
      // }
    }
  }
