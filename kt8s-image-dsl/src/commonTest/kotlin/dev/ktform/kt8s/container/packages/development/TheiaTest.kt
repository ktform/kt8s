...
class TheiaTest {
  ...
  fun testTheia() {
    runTest(timeout = 10.seconds) {
      val latest = Theia.`package`.latestVersion().getOrElse { err -> throw Exception("Unable to determine latest version: $err") }

      Environment.all.forEach { env ->
        PackageTestCase(
          "theia",
          env,
          rendered = Theia(latest).render().getOrElse { err -> throw Exception("Unable to render: $err") },
        ).isExpected()
      }
    }
  }
