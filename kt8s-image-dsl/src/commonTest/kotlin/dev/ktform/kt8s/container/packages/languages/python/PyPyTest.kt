...
class PyPyTest {
  ...
  fun testPyPy() {
    runTest(timeout = 10.seconds) {
      ...
      Environment.all.forEach { env ->
        PackageTestCase("py py", env, rendered = PyPy(latest).render().getOrElse { err ->throw Exception("Unable to render: $err") }).isExpected()
      }
    }
  }
  ...
  fun testPyPyLatestVersions() {
    runTest(timeout = 10.seconds) {
      val latestNVersions = PyPy.`package`.availableVersions(Environment.default).getOrElse { err -> throw Exception("Unable to determine available versions: $err") }
