...
class FeastTest {
    ...
    fun testFeast() {
        runTest(timeout = 10.seconds) {
            assertThat(FeastVersionFetcher.getLatestVersions()).isNotEmpty()

            FeastVersionFetcher.getLatestVersions().forEach { (component, version) ->
                val cli =
                    when (component) {
                        is FeastComponent if (component == FeastComponent.Feast) -> Feast(version)
