...
class ExternalDnsTest {
    ...
    fun testExternalDns() {
        runTest(timeout = 10.seconds) {
            assertThat(ExternalDnsVersionFetcher.getLatestVersions()).isNotEmpty()

            ExternalDnsVersionFetcher.getLatestVersions().forEach { (component, version) ->
                val externalDNS =
                    when (component) {
                        is ExternalDnsComponent if
                            (component == ExternalDnsComponent.ExternalDns)
                         -> ExternalDns(version)
