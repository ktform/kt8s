...
class GrypeTest {
    ...
    fun testGrype() {
        runTest(timeout = 10.seconds) {
            assertThat(GrypeVersionFetcher.getLatestVersions()).isNotEmpty()

            GrypeVersionFetcher.getLatestVersions().forEach { (component, version) ->
                val grype =
                    when (component) {
                        is GrypeComponent if (component == GrypeComponent.Grype) -> Grype(version)
