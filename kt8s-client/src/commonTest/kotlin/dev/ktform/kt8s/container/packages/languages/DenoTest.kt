...
class DenoTest {
    ...
    fun testDeno() {
        runTest(timeout = 10.seconds) {
            assertThat(DenoVersionFetcher.getLatestVersions()).isNotEmpty()

            DenoVersionFetcher.getLatestVersions().forEach { (component, version) ->
                val cli =
                    when (component) {
                        is DenoComponent if (component == DenoComponent.Deno) -> Deno(version)
