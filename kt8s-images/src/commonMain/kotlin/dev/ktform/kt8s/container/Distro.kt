...
sealed class Distro(
  ...
  companion object {
    ...
    fun List<Package>.forNonRPMDistros() = nonRpm.entries.associate { (_, distro) -> distro to this }

    fun Map<Distro, List<Package>>.plus(other: Map<Distro, List<Package>>) =
      this.entries.associate { (k, v) -> k to v + other[k].orEmpty() }

    val nonRpm = mapOf<String, Distro>(
      "alpine" to Alpine,
      "debian" to Debian,
    )

    val rpm = mapOf<String, Distro>(
      "photon" to Photon,
      "oracle" to Oracle,
      "rocky" to Rocky,
      "ubi" to UBI,
