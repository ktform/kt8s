...
class DockerfileTest {
    ...
    @Test
    fun testFromInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
    ...
    @Test
    fun testWorkdirInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf("absolute path" to { workdir("/app") }, "relative path" to { workdir("src") })
    ...
    @Test
    fun testEnvInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
                "single variable" to { env("NODE_ENV", "production") },
                "multiple variables using vararg" to
                    {
                        env("NODE_ENV" to "production", "PORT" to "3000")
                    },
            )
    ...
    @Test
    fun testEntrypointInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
    ...
    @Test
    fun testLabelInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
                "single label" to { label("version", "1.0") },
                "multiple labels" to
                    {
                        label("version" to "1.0", "maintainer" to "team@example.com")
                    },
            )
    ...
    @Test
    fun testHealthcheckInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
                "disable healthcheck" to { healthcheck() },
                "simple healthcheck" to
                    {
    ...
    @Test
    fun testStopsignalInstructions() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf("SIGTERM" to { stopsignal("SIGTERM") }, "SIGKILL" to { stopsignal("SIGKILL") })
    ...
    fun testRunAdvancedSemantics() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
                "with bind mount" to
                    {
                        run(
                            "make build",
                            listOf(Dockerfile.BindMount("/host/src", "/app/src", readonly = true)),
                        )
                    },
                "with cache mount" to
                    {
                ...
                "with secret mount" to
                    {
                        run(
                            $$"echo $SECRET",
                            listOf(
                                Dockerfile.SecretMount("mysecret", target = "/run/secrets/mysecret")
                            ),
                        )
                    },
                "with ssh mount" to
                ...
                "with network none" to { run("go build .", network = Dockerfile.RunNetwork.NONE) },
    ...
    @Test
    fun testRunDslConfiguration() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
                "with bind mount using DSL" to
                    {
                        run("make build") { bindMount("/host/src", "/app/src", readonly = true) }
                    },
                ...
                "with tmpfs mount using DSL" to
                    {
                        run("npm test") { tmpfsMount("/tmp", size = "100m", mode = "1777") }
                    },
    ...
    @Test
    fun testFluentDslConfiguration() {
        val testCases: Map<String, Dockerfile.() -> Unit> =
            mapOf(
