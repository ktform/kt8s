...
data class KubernetesResource(
  ...
  val typeSpec: TypeSpec by lazy {
    val constructorFields = fields.filterNot { it.name in setOf("apiVersion", "kind", "group", "version") }
  ...
  private fun TypeSpec.Builder.addKDoc(fields: List<KubernetesResourceField>) = apply {
    val docs = fields.filter { it.description.isNotBlank() }
  ...
  private fun metadataProperty(name: String, vararg args: Any, serialized: Boolean, format: String = "%S") =
    PropertySpec.builder(name, STRING)
      .addModifiers(KModifier.OVERRIDE)
      .initializer(format, *args)
      .addAnnotation(if (serialized) {
        AnnotationSpec.builder(ClassName("kotlinx.serialization", "SerialName"))
          .addMember(format, name).build()
      } else {
  ...
  internal fun getFileBuilder(): FileSpec.Builder =
    FileSpec.builder(packageName, kind)
      .addType(typeSpec)
      .addImport("dev.ktform.kt8s.resources",
        "IntOrString", "RawJsonObject", "StringOrNumber", "KubernetesTime", "KubernetesMicroTime", "Resource")

  override fun toString(): String = getFileBuilder().build().toString()

  companion object {
    fun fromJsonSchema(definition: JSONSchema.Definition, all: List<JSONSchema.Definition>, packageName: String): KubernetesResource {
      val (group, version, kind) = definition.kubernetesGroupVersionKind.firstOrNone().getOrElse { throw IllegalArgumentException("Failed to parse kubernetesGroupVersionKind: ${definition.name}") }
      ...
      return KubernetesResource(
        ...
        fields = definition.properties.map { KubernetesResourceField.fromJsonSchema(it, all) }
      )
    }
  }
}
