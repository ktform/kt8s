...
class JSONSchemaTest : FunSpec({
  ...
  context("schema parsing and validation") {
    withData(
      nameFn = { it.name },
      ...
      JSONSchema.parseSchemaString(testCase.schema).fold(
        ifLeft = { error ->
          if (testCase.expectError) {
            testCase.expectedErrorType?.invoke(error)?.shouldBeTrue()
        ...
        ifRight = { definitions ->
          if (testCase.expectError) {
            ...
            definitions shouldHaveSize testCase.expectedDefinitionCount
            testCase.validate(definitions) shouldBeRight Unit
          }
        }
      )
    }
  ...
  test("all schema versions should be loadable") {
