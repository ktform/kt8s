...
/**
 * @param expirationSeconds expirationSeconds is the requested duration of validity of the issued
 *   certificate. The certificate signer may issue a certificate with a different validity duration
 *   so a client must check the delta between the notBefore and and notAfter fields in the issued
 ...
 *
 * @param extra extra contains extra attributes of the user that created the
 *   CertificateSigningRequest. Populated by the API server on creation and immutable.
 * @param groups groups contains group membership of the user that created the
 *   CertificateSigningRequest. Populated by the API server on creation and immutable.
 * @param request request contains an x509 certificate signing request encoded in a "CERTIFICATE
 *   REQUEST" PEM block. When serialized as JSON or YAML, the data is additionally base64-encoded.
 ...
 *    authenticate to kube-apiserver. Requests for this signer are never auto-approved by
 ...
 *    authenticate to kube-apiserver. Requests for this signer can be auto-approved by the
 *    "csrapproving" controller in kube-controller-manager, and can be issued by the "csrsigning"
 ...
 *    endpoints, which kube-apiserver can connect to securely. Requests for this signer are never
 *    auto-approved by kube-controller-manager, and can be issued by the "csrsigning" controller in
 *    kube-controller-manager.
 *
 ...
 *
 * @param uid uid contains the uid of the user that created the CertificateSigningRequest. Populated
 ...
 * "server auth".
 *
 * Valid values are: "signing", "digital signature", "content commitment", "key encipherment", "key
 * agreement", "data encipherment", "cert sign", "crl sign", "encipher only", "decipher only",
 * "any", "server auth", "client auth", "code signing", "email protection", "s/mime", "ipsec end
 * system", "ipsec tunnel", "ipsec user", "timestamping", "ocsp signing", "microsoft sgc", "netscape
 * sgc"
 *
 * @param username username contains the name of the user that created the
 *   CertificateSigningRequest. Populated by the API server on creation and immutable.
 */
@Serializable
public data class CertificateSigningRequestSpec(
    public val expirationSeconds: Int,
    ...
    @SerialName("kind") override val kind: String = "CertificateSigningRequestSpec"
}
