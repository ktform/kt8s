...
/**
 * @param borrowingLimitPercent `borrowingLimitPercent`, if present, configures a limit on how many
 *   seats this priority level can borrow from other priority levels. The limit is known as this
 *   level's BorrowingConcurrencyLimit (BorrowingCL) and is a limit on the total number of seats
 *   that this level may borrow at any one time. This field holds the ratio of that limit to the
 *   level's nominal concurrency limit. When this field is non-nil, it must hold a non-negative
 ...
 *   can be borrowed by other priority levels. The value of this field must be between 0 and 100,
 ...
 *
 * @param limitResponse `limitResponse` indicates what to do with requests that can not be executed
 *   right now
 ...
 *   available at this priority level. This is used both for requests dispatched from this priority
 *   level as well as requests dispatched from other priority levels borrowing seats from this
 *   level. The server's concurrency limit (ServerCL) is divided among the Limited priority levels
 ...
 */
@Serializable
public data class LimitedPriorityLevelConfiguration(
    public val borrowingLimitPercent: Int,
    public val lendablePercent: Int,
    public val limitResponse: LimitResponse,
    ...
    @SerialName("kind") override val kind: String = "LimitedPriorityLevelConfiguration"
}
