 ...
 *   Note the following deviations from the "host" part of the URI as defined in RFC 3986: 1. IPs
 *   are not allowed. Currently an IngressRuleValue can only apply to the IP in the Spec of the
 *   parent Ingress.
 * 2. The `:` delimiter is not respected because ports are not allowed. Currently the port of an
 *    Ingress is implicitly :80 for http and :443 for https. Both these may change in the future.
 *    Incoming requests are matched against the host before the IngressRuleValue. If the host is
 ...
 * be matched against the Host field in the following way: 1. If host is precise, the request
 * matches this rule if the http host header is equal to Host. 2. If host is a wildcard, then the
 * request matches this rule if the http host header is to equal to the suffix (removing the first
 * label) of the wildcard rule.
 */
@Serializable
public data class IngressRule(public val host: String, public val http: HTTPIngressRuleValue) :
    Resource {
    ...
    @SerialName("kind") override val kind: String = "IngressRule"
}
