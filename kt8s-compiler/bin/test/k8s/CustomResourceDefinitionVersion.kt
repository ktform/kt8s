...
/**
 * @param additionalPrinterColumns additionalPrinterColumns specifies additional columns returned in
 *   Table output. See
 ...
 *   Defaults to false.
 * @param deprecationWarning deprecationWarning overrides the default warning returned to API
 *   clients. May only be set when `deprecated` is true. The default warning indicates this version
 *   is deprecated and recommends use of the newest served version of equal or greater stability, if
 *   one exists.
 * @param name name is the version name, e.g. “v1”, “v2beta1”, etc. The custom resources are served
 *   under this version at `/apis/<group>/<version>/...` if `served` is true.
 * @param schema schema describes the schema used for validation, pruning, and defaulting of this
 *   version of the custom resource.
 * @param selectableFields selectableFields specifies paths to fields that may be used as field
 ...
 * @param storage storage indicates this version should be used when persisting custom resources to
 *   storage. There must be exactly one version with storage=true.
 * @param subresources subresources specify what subresources this version of the defined custom
 *   resource have.
 */
@Serializable
public data class CustomResourceDefinitionVersion(
    ...
    @SerialName("kind") override val kind: String = "CustomResourceDefinitionVersion"
}
