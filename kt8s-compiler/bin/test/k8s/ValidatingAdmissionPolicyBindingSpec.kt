...
/**
 * @param matchResources MatchResources declares what resources match this binding and will be
 *   validated by it. Note that this is intersected with the policy's matchConstraints, so only
 *   requests that are matched by the policy can be selected by this. If this is unset, all
 *   resources matched by the policy are validated by this binding When resourceRules is unset, it
 *   does not constrain resource matching. If a resource is matched by the other fields of this
 ...
 *   policy. It should point to a resource of the type specified in ParamKind of the bound
 ...
 *   ValidatingAdmissionPolicy applied. If the policy does not specify a ParamKind then this field
 *   is ignored, and the rules are evaluated without a param.
 * @param policyName PolicyName references a ValidatingAdmissionPolicy name which the
 *   ValidatingAdmissionPolicyBinding binds to. If the referenced resource does not exist, this
 *   binding is considered invalid and will be ignored Required.
 * @param validationActions validationActions declares how Validations of the referenced
 *   ValidatingAdmissionPolicy are enforced. If a validation evaluates to false it is always
 *   enforced according to these actions.
 *
 * Failures defined by the ValidatingAdmissionPolicy's FailurePolicy are enforced according to these
 * actions only if the FailurePolicy is set to Fail, otherwise the failures are ignored. This
 ...
 * audit annotation with a value containing the details of the validation failures, formatted as a
 * JSON list of objects, each with the following fields: - message: The validation failure message
 * string - policy: The resource name of the ValidatingAdmissionPolicy - binding: The resource name
 * of the ValidatingAdmissionPolicyBinding - expressionIndex: The index of the failed validations in
 * the ValidatingAdmissionPolicy - validationActions: The enforcement actions enacted for the
 * validation failure Example audit annotation:
...
public data class ValidatingAdmissionPolicyBindingSpec(
    ...
    @SerialName("kind") override val kind: String = "ValidatingAdmissionPolicyBindingSpec"
}
