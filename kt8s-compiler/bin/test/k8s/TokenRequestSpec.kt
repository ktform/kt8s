...
/**
 * @param audiences Audiences are the intendend audiences of the token. A recipient of a token must
 *   identify themself with an identifier in the list of audiences of the token, and otherwise
 *   should reject the token. A token issued for multiple audiences may be used to authenticate
 ...
 *   The token will only be valid for as long as the bound object exists. NOTE: The API server's
 ...
 *   The token issuer may return a token with a different validity duration so a client needs to
 *   check the 'expiration' field in a response.
 */
@Serializable
public data class TokenRequestSpec(
    ...
    @SerialName("kind") override val kind: String = "TokenRequestSpec"
}
