...
/**
 * @param lastTransitionTime `lastTransitionTime` is the last time the condition transitioned from
 *   one status to another.
 * @param message `message` is a human-readable message indicating details about last transition.
 * @param reason `reason` is a unique, one-word, CamelCase reason for the condition's last
 *   transition.
 ...
 */
@Serializable
public data class PriorityLevelConfigurationCondition(
    ...
    @SerialName("kind") override val kind: String = "PriorityLevelConfigurationCondition"
}
