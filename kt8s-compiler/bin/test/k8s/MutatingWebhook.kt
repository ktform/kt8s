...
/**
 * @param admissionReviewVersions AdmissionReviewVersions is an ordered list of preferred
 *   `AdmissionReview` versions the Webhook expects. API server will try to use first version in the
 *   list which it supports. If none of the versions specified in this list supported by API server,
 *   validation will fail for this object. If a persisted webhook configuration specifies allowed
 *   versions and does not include any versions known to the API Server, calls to the webhook will
 ...
 * @param failurePolicy FailurePolicy defines how unrecognized errors from the admission endpoint
 *   are handled - allowed values are Ignore or Fail. Defaults to Fail.
 * @param matchConditions MatchConditions is a list of conditions that must be met for a request to
 *   be sent to this webhook. Match conditions filter requests that have already been matched by the
 *   rules, namespaceSelector, and objectSelector. An empty list of matchConditions matches all
 ...
 *
 * @param matchPolicy matchPolicy defines how the "rules" list is used to match incoming requests.
 ...
 *   `apiGroups:["apps"], apiVersions:["v1"], resources: ["deployments"]`, a request to apps/v1beta1
 ...
 *
 * @param name The name of the admission webhook. Name should be fully qualified, e.g.,
 ...
 * @param namespaceSelector NamespaceSelector decides whether to run the webhook on an object based
 ...
 * of "0" or "1"; you will set the selector as follows: "namespaceSelector": { "matchExpressions":
 ...
 *
 * @param objectSelector ObjectSelector decides whether to run the webhook based on if the object
 *   has matching labels. objectSelector is evaluated against both the oldObject and newObject that
 *   would be sent to the webhook, and is considered to match if either object matches the selector.
 *   A null object (oldObject in the case of create, or newObject in the case of delete) or an
 *   object that cannot have labels (like a DeploymentRollback or a PodProxyOptions object) is not
 *   considered to match. Use the object selector only if the webhook is opt-in, because end users
 *   may skip the admission webhook by setting the labels. Default to the empty LabelSelector, which
 *   matches everything.
 * @param reinvocationPolicy reinvocationPolicy indicates whether this webhook should be called
 *   multiple times as part of a single admission evaluation. Allowed values are "Never" and
 *   "IfNeeded".
 *
 ...
 * Defaults to "Never".
 *
 * @param rules Rules describes what operations on what resources/subresources the webhook cares
 *   about. The webhook cares about an operation if it matches _any_ Rule. However, in order to
 *   prevent ValidatingAdmissionWebhooks and MutatingAdmissionWebhooks from putting the cluster in a
 ...
 * @param sideEffects SideEffects states whether this webhook has side effects. Acceptable values
 *   are: None, NoneOnDryRun (webhooks created via v1beta1 may also specify Some or Unknown).
 *   Webhooks with side effects MUST implement a reconciliation system, since a request may be
 *   rejected by a future step in the admission chain and the side effects therefore need to be
 *   undone. Requests with the dryRun attribute will be auto-rejected if they match a webhook with
 *   sideEffects == Unknown or Some.
 * @param timeoutSeconds TimeoutSeconds specifies the timeout for this webhook. After the timeout
 *   passes, the webhook call will be ignored or the API call will fail based on the failure policy.
 *   The timeout value must be between 1 and 30 seconds. Default to 10 seconds.
 */
@Serializable
public data class MutatingWebhook(
    public val admissionReviewVersions: List<String>,
    public val clientConfig: WebhookClientConfig,
    public val failurePolicy: String,
    public val matchConditions: List<MatchCondition>,
    public val matchPolicy: String,
    public val name: String,
    public val namespaceSelector: LabelSelector,
    public val objectSelector: LabelSelector,
    public val reinvocationPolicy: String,
    public val rules: List<RuleWithOperations>,
    public val sideEffects: String,
    ...
    @SerialName("kind") override val kind: String = "MutatingWebhook"
}
