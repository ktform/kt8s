...
class KubernetesResourceCodegenTest {
  ...
  fun testResources() {
    ...
    val defs = JSONSchema.load(schema).getOrElse { throw IllegalStateException("Failed to load schema $schema") }

    defs.toResources("dev.ktform.kt8s.resources").forEach { res ->
      val actual = res.toString().trimIndent().trim()
      assertThat(actual).isEqualTo(actual.getOrUpdateExpected(Path("$targetDir/${res.kind}.kt")))
    }
  }
}
